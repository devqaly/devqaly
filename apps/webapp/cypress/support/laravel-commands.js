...
Cypress.Commands.add('currentUser', () => {
  return cy.then((token) => {
    return cy
      .request({
        method: 'POST',
        url: Cypress.env('BACKEND_API_URL') + '/__cypress__/current-user',
        body: { _token: token },
        log: false
      })
      .then((response) => {
...
Cypress.Commands.add('logout', () => {
  return cy
    .then((token) => {
      return cy.request({
        method: 'POST',
        url: Cypress.env('BACKEND_API_URL') + '/__cypress__/logout',
        body: { _token: token },
        log: false
      })
    })
...
Cypress.Commands.add('refreshRoutes', () => {
  return cy.then((token) => {
    return cy
      .request({
        method: 'POST',
        url: Cypress.env('BACKEND_API_URL') + '/__cypress__/routes',
        body: { _token: token },
        log: false
      })
      .its('body', { log: false })
...
Cypress.Commands.add('create', (model, count = 1, attributes = {}, load = [], state = []) => {
  ...
  return cy
    .then((token) => {
      return cy.request({
        method: 'POST',
        url: Cypress.env('BACKEND_API_URL') + '/__cypress__/factory',
...
Cypress.Commands.add('artisan', (command, parameters = {}, options = {}) => {
  ...
  return cy.then((token) => {
    return cy.request({
      method: 'POST',
      url: Cypress.env('BACKEND_API_URL') + '/__cypress__/artisan',
...
Cypress.Commands.add('php', (command) => {
  return cy
    .then((token) => {
      return cy.request({
        method: 'POST',
        url: Cypress.env('BACKEND_API_URL') + '/__cypress__/run-php',
